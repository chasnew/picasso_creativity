...
# rows where month and date aren't specified (6378 entries)
...
art_byYear <- art_data %>% 
  group_by(yearEnd, category) %>% 
  summarize(n = n(), .groups="drop") 

# expand years in each category and fill w/ 0
art_byYear <- art_byYear %>% 
  complete(expand(art_byYear, yearEnd, category), fill=list(n = 0)) %>%
  mutate(any_artworks = n > 0)

art_byYear %>% 
  ggplot(aes(x=yearEnd, y=n, group=category, color=category, fill=any_artworks)) + 
  geom_point(shape=21) + 
...
majorPeriodOnset <- tibble(period = c("Blue",
  ...
  mutate(timeWindow = ceiling(yearMonthStart / windowSize) * windowSize,
         onsetYear = floor(timeWindow), dummy_loc = "nimes") # dummy_loc determines the position of labels
...
art_byMonth.ent %>%
  ...
  scale_y_continuous("entropy (variation of category)") +
  theme_classic()
...
artloc.ent %>%
  ...
  geom_point() +
  scale_x_continuous("start year") +
  scale_y_continuous("entropy (variation of location)") +
  theme_classic()
...
merge.ent %>%
  ggplot(aes(x = timeWindow, y = ent.smooth.annual.x)) +
  geom_line() +
  geom_line(aes(x = timeWindow, y = ent.smooth.annual.y)) +
  theme_classic()
```
