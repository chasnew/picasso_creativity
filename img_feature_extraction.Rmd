...
# (None, 14, 14, 256)
model.conv4 <- keras_model(inputs = base_model$input,
                           outputs = get_layer(base_model, 'conv4_block1_1_conv')$output)
# (None, 2048)
model.avgpool <- keras_model(inputs = base_model$input,
...
ggplot() +
  geom_point(aes(x = 1:336, y = low.pca.results$sdev)) +
...
ggplot() +
  geom_point(aes(x = 1:3367, y = high.pca.results$sdev)) +
  geom_vline(xintercept = 100, color = "blue") +
  xlab("pca dimension") +
  ylab("explained sdev") +
...
save(pca_sd_list, file = "pca_sd_list.Rdata")
```
