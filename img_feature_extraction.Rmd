...
reduced_art <- art_data %>% 
  select(opp, title, yearStart, yearMonthStart)
...
reconstruct_opp <- function(str_vector) {
  return(paste0("OPP.", substr(str_vector[1], 5, 6), ":", substr(str_vector[2], 1, 3)))
}

sample_images %>%
  str_split("-") %>% 
  sapply(reconstruct_opp)
...
year_list <- list.files(img_dir)

sample_images <- list.files("sample_images")
full_paths <- paste0("sample_images/", sample_images)
...
nz_masks <- sample_features %>% 
  colSums != 0
sample_features <- sample_features[,nz_masks]
```

# Dimensionality reduction
```{r}
# one row with no match
sample_df <- data.frame(opp = image_opps) %>% 
  left_join(reduced_art, by = "opp")

sample_df[39,"yearStart"] = 1923
sample_df[39,"yearMonthStart"] = 1923.5

# PCA analysis
low_pca.results <- prcomp(low_features,
                          center = TRUE,
                          scale. = TRUE)
sample_df <- cbind(sample_df, low_pca.results$x[,1:2]) %>% 
  rename(low_pc1 = "PC1", low_pc2 = "PC2")

high_pca.results <- prcomp(high_features,
                           center = TRUE,
                           scale. = TRUE)
sample_df <- cbind(sample_df, high_pca.results$x[,1:2]) %>% 
  rename(high_pc1 = "PC1", high_pc2 = "PC2")

pca.results <- prcomp(sample_features,
                      center = TRUE,
                      scale. = TRUE)
sample_df <- cbind(sample_df, pca.results$x[,1:2]) %>% 
  rename(full_pc1 = "PC1", full_pc2 = "PC2")

# t-SNE analysis
low_tsne <- low_features %>% Rtsne(perplexity = 10)
low_tsne <- low_tsne$Y %>% as.data.frame() %>% rename(low_tsne1 = "V1", low_tsne2 = "V2")

high_tsne <- high_features %>% Rtsne(perplexity = 10)
high_tsne <- high_tsne$Y %>% as.data.frame() %>% rename(high_tsne1 = "V1", high_tsne2 = "V2")

full_tsne <- sample_features %>% Rtsne(perplexity = 10)
...
ggplot(data = sample_df, aes(x = full_pc1, y = full_pc2, label = opp, color = yearMonthStart)) +
  geom_point() +
  geom_text_repel(size = 2.5) +
