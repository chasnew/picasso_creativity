...
```{r movement analysis}
ordered_df <- full_df %>% 
  left_join(art_data %>% select(opp, dayStart),
            by = "opp") %>% 
  filter(monthStart != 0, dayStart != 0) %>% 
  arrange(yearStart, monthStart, dayStart) %>% 
  select(yearStart, monthStart, low_pc1:high_pc100)
...
trimmed_pca2 <- d_ordered_pca[-nrow(ordered_df),] # remove the last row
...
```{r}
ordered_df$cos_sim <- c(NA, NA, d_cos_sims)

ordered_df <- ordered_df %>% 
  mutate(decadeStart = floor(yearStart/10)*10)
...
```{r}
slidw_df <- read_csv(file.path(picasso_path, "results", "slide_window12m.csv"))
...
```{r}
# sd of mean vs mean of sd
resample_nslidw <- bootstraps(nslidw_df[c(3,5)], times = 1000)

# bootstrapping step sizes
bootstp_stsz <- map_dbl(
  resample_nslidw$splits,
  function(split) {
    steps <- as.data.frame(split) %>% 
      filter(!is.na(step_avg)) %>% 
      pull(step_avg)
    mean(steps)
  }
)
...
# bootstrapping direction changes
bootstp_dir <- map_dbl(
  resample_nslidw$splits,
  function(split) {
    direct <- as.data.frame(split) %>% 
      filter(!is.na(dir_avg)) %>% 
      pull(dir_avg)
    mean(direct)
  }
)
