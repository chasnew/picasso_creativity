...
ggsave("img/full_tsne_artviz.png")
...
ggsave("img/full_tsne_artistic_movement.png")
```
...
fclust_samples %>% 
  filter(full_clust %in% c(1:5, 51, 200)) %>% 
...
## Rank order for step size and directional change in stylistic space
...
consec.cossim <- function(vector1, vector2) {
  return(NA)
}
...
ggsave("img/direct_change_dist.png")
```
